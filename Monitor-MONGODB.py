...
def connect_to_mongodb(uri):
    ...
    client = MongoClient('<string de conecxao do mongodb>')
    db = client.admin
    return db
...
def display_server_stats(stats, db, fields, output_file, header=True):
    ...
    metrics = {
        ...
        'CursorsTotal': stats['metrics']['atlas']['connectionPool']['totalCreated'],
        #'Timestamp': str(stats['operationTime']).split('(')[1].split(')')[0].split(',')[0]
